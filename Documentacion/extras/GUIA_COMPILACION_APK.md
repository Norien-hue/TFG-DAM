...
```powershell
cd ReciApp_Final\App_Movil\source
npm install
```
...
```powershell
cd ReciApp_Final\App_Movil\source\android
...
### 3.3 Copiar la APK a la carpeta compilado

```powershell
Copy-Item "android\app\build\outputs\apk\release\app-release.apk" "ReciApp_Final\App_Movil\compilado\ReciApp-release.apk"
```
...
# === COPIAR A LA CARPETA COMPILADO ===
Copy-Item "app\build\outputs\apk\release\app-release.apk" "..\..\compilado\ReciApp-release.apk"

# === INSTALAR EN EMULADOR ===
& "$env:LOCALAPPDATA\Android\Sdk\platform-tools\adb.exe" install -r "..\..\compilado\ReciApp-release.apk"
```
